...
今年も大盛り上がりな [ISUCON 7](http://isucon.net/archives/50949022.html) でしたが、わたしも†空中庭園†《ガーデンプレイス》というチームで、同僚の [@ryot_a_rai](https://twitter.com/ryot_a_rai) さんと [@eagletmt](https://twitter.com/eagletmt) さんと一緒に予選に出場しました。
...
### :arrow_up: フロントの Nginx の設定でアイコンをキャッシュする
...
### :computer: srv3 にも Nginx を置いてリクエストを受けるようにする
...
### :chart: キャッシュコントロールを仕込んでブレイクスルーが起きる
...
### :hocho: Kill N+1、徐々に MySQL から Redis にデータを逃す
...
とはいえ少しでもスコアを上げたいため、最期の仕上げとしてプロファイラを外して各種ログを切ってみたところスコアが 590,000 ほどまで伸びました。特に効いたのが Nginx のログのようで、アクセスログを書き出すのも結構 CPU を使っているのだな〜という気づきがありました。
...
とはいえやはり @ryot_a_rai さんと @eagletmt さんが非常に頼りになるということもあり、終始おんぶにだっこだったと思います。本戦まで 1 ヶ月ほどあるので、個人練習を重ねてもっと動けるようになれるように準備しようと思っています :muscle: この場をお借りして、チームメンバーのふたりにはお礼申し上げます。

そして ISUCON 7 のスタッフの皆様、本当に素晴らしい問題を提供してくださり、ありがとうございました。その苦労を考えると、ほんとうに頭の下がる思いです :bow:

それではみなさん、ミライナタワーで会いましょう :wave:
